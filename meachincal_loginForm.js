 // --- DOM ELEMENT SELECTION ---
        ...
        const successCheck = document.querySelector('.success-message path');
        ...
        // Add a subtle idle animation to the machine
        gsap.to(scalesLine, {
            rotation: 1,
            svgOrigin: "92 20",
            duration: 2,
        ...
        submitBtn.addEventListener('click', (e) => {
            ...
            if (emailValid && checkboxEl.checked && nameValid && sprayRepeatCounter > 1) {
                ...
                gsap.fromTo(formContainer, { x: 0 }, {
                    duration: 0.5,
                    x: 10,
                    ease: "elastic.out(1, 0.2)",
                    onComplete: () => gsap.set(formContainer, { x: 0 })
                });
            }
        })
        ...
        function playSuccessAnimation() {
            ...
            // 1. Machine goes into overdrive
            tl.to(gearsTls, { timeScale: 5, duration: 0.5, ease: 'power2.in' })
              .to('svg', { filter: 'brightness(1.5)', duration: 0.5 }, 0);
            ...
            // 3. SVG machine fades out after a delay
            tl.to('svg', {
                duration: 1,
                opacity: 0,
                scale: 0.8,
            ...
            // 4. Success checkmark animates in
            const checkLength = successCheck.getTotalLength();
            gsap.set(successCheck, { strokeDasharray: checkLength, strokeDashoffset: checkLength });
            
            tl.to(successMessage, { opacity: 1, duration: 0.2 }, ">-0.5")
              .to(successCheck, { strokeDashoffset: 0, duration: 0.8, ease: 'power2.out' });
        }
        ...
        function layoutPreparation() {
            ...
            gsap.set(timingChains[0], { attr: { "stroke-dasharray": "0 12" } })
            gsap.set(timingChains[1], { attr: { "stroke-dasharray": "0 12" } })
        ...
        function createEmailTl() {
            ...
            .fromTo(scalesLine, { rotation: -20, }, { duration: .15, rotation: -1, svgOrigin: "92 20" }, hammerTimeStart)
            ...
            .fromTo(car, { y: 0 }, { y: -5, duration: 0.1, yoyo: true, repeat: 5, ease: "power1.inOut"}, hammerTimeStart + 0.1);
            ...
            tl.progress(0.001);
            tl.reverse(); // Start in reversed state
            return tl;
        }

        function createGearsTimelines() {
            const tls = []
            const params = { baseSize: 15, pitch: 11, teethCurve: .6, startPos: {x: 634, y: 389}, speed: .2 }
            const data = [{ angle: 0, teethNumber: 10, hasHole: true }, { angle: -.5, teethNumber: 32, hasHole: true }, { angle: 1.65, teethNumber: 12, hasHole: false }];
            ...
            data.forEach((d, dIdx) => {
                ...
                let x, y, startAngle;
                if (dIdx === 0) {
                    startAngle = 0; x = params.startPos.x; y = params.startPos.y;
                } else {
                ...
                gearsContainer.appendChild(group); group.appendChild(path);
                const gear = { idx: dIdx, center: {x, y}, radius, angle: startAngle, teethNumber: d.teethNumber, hasHole: d.hasHole, toothAngle: 2 * Math.PI / d.teethNumber, toothCurveAngle: params.teethCurve / d.teethNumber, group }
                const rOut = gear.radius + .25 * params.pitch; const rIn = rOut - .75 * params.pitch;
                ...
                if (dIdx === 0) {
                    ...
                    gsap.set(circle, { attr: { cx: gear.center.x, cy: gear.center.y, r: 5, fill: "#e94560" } })
                    gearsContainer.appendChild(circle);
                    gsap.set(path, { attr: { fill: "#0f3460", "fill-opacity": .5 } })
                } else if (dIdx === (data.length - 1)) {
                    gsap.set(path, { attr: { fill: "#0f3460", "fill-opacity": .5 } })
                    const circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
                    gsap.set(circle, { attr: { cx: gear.center.x + handleRadius, cy: gear.center.y, r: 5, fill: "#e94560" } })
                    gear.group.appendChild(circle);
                }
                path.setAttribute("d", pathD);
                const tl = gsap.timeline({ repeat: -1, paused: true, })
                    .to(group, { duration: params.speed * gear.teethNumber, rotation: 360 * (gear.idx % 2 ? -1 : 1), svgOrigin: gear.center.x + " " + gear.center.y, ease: "none", });
                if (dIdx === (data.length - 1)) {
                    tl.eventCallback("onUpdate", () => {
                        ...
                        if (deltaY > 8) {
                            ...
                            let sprayProgress = Math.max(0, tl.progress() - .1) * (1 / .2);
                            let bubblesOpacity = (sprayProgress > 1) ? 0 : sprayProgress * (1 - Math.pow(sprayProgress, 8));
                        ...
                        gsap.set(gearConnector, {
                            attr: { x1: gear.center.x + handleRadius * Math.cos(angle), y1: gear.center.y + handleRadius * Math.sin(angle), x2: 700 + 18, y2: 646 - 100 + deltaY }
                        })
                    });
                ...
                tl.progress(0.6)
                tls.push(tl); gears.push(gear);
            })
            return tls;
        }

        function createPullingTimeline(isFixed, BtnPulled) {
            let tl = gsap.timeline({ defaults: { ease: "power1.inOut", duration: 1 }, onUpdate: animatePullingLine });
            if (isFixed && BtnPulled) {
                tl.to(state, { pullProgress: 1 }, 0).to(submitBtn, { rotation: 0 }, 0).to(state, { duration: .1, sumbitBtnOnPlace: 1 }, .9)
                  .to(checkboxPullLine, { attr: {y2: 44 - 130} }, 0).to(checkboxPullCircle, { y: 44 - 130 }, 0)
            } else if (!isFixed && BtnPulled) {
                tl.to(state, { pullProgress: 1 }, 0).to(checkboxPullLine, { attr: {y2: 44 - 130} }, 0).to(checkboxPullCircle, { y: 44 - 130 }, 0)
            } else if (isFixed && !BtnPulled) {
                tl.to(state, { pullProgress: 0 }, 0).to(submitBtn, { rotation: -90 }, 0).to(state, { duration: .1, sumbitBtnOnPlace: 0 }, 0)
                  .to(checkboxPullLine, { attr: {y2: 44} }, 0).to(checkboxPullCircle, { y: 44 }, 0)
            } else if (!isFixed && !BtnPulled) {
                tl.to(state, { pullProgress: 0 }, 0).to(checkboxPullLine, { attr: {y2: 44} }, 0).to(checkboxPullCircle, { y: 44 }, 0)
            }
            function animatePullingLine() {
                const buttonOriginPoint = [260, -76]; const btnWidth = 270;
                const deg = (gsap.getProperty(submitBtn, "rotation") - 4) * Math.PI / 180;
                const btnEnd = [ buttonOriginPoint[0] - (btnWidth - 20) * Math.cos(deg), buttonOriginPoint[1] - (btnWidth - 20) * Math.sin(deg), ]
                gsap.set(btnHandlerCircle, { attr: { cx: btnEnd[0], cy: btnEnd[1] } })
                const handle = 7; const r = 10;
                let btnPullLinePath = "M" + (-r - handle) + "," + (250 - (isFixed ? 0 : state.pullProgress * 300));
                btnPullLinePath += "h" + (2 * handle) + "h" + (-handle) + " V" + (44 - state.pullProgress * 130);
                const slideAngle = .3 * Math.PI * (1 - (isFixed ? 1 : .5) * state.pullProgress);
                const dx = r * Math.cos(slideAngle); const dy = -r * Math.sin(slideAngle);
